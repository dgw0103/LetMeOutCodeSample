...
public class SlidePuzzle : CameraFixingComponent
{
    [SerializeField] private GameObject glitchProductionTrigger;
    [SerializeField] private PuzzleSolvingDataSaver puzzleSolvingDataSaver;
    ...
    {
        gameObject.SetLayerIncludingChildren(Keyword.defaultText);
        if (Selections.TryPeek(out Selection selection) && selection.Equals(this))
        {
            OnUnselection();
        }
        StartCoroutine(nameof(PlayDisappearingBooksAndProductionCoroutine));
        puzzleSolvingDataSaver.IsSolved = true;
    }
    private IEnumerator PlayDisappearingBooksAndProductionCoroutine()
    {
        yield return FindObjectOfType<BookShelfDoorController>().DisappearBooks();
        Destroy(this);
        Destroy(GetComponent<PuzzleDeviceConverter>());
        glitchProductionTrigger.SetActive(true);
    }
